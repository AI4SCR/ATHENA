...
def add_node_features(so,
    ...
    Args:
        ...
        - `feature_type`: String specifying the type of parameters to assign. 
    ...
    try:
        ...
        raise KeyError(f'Either spl: "{spl}" or graph_key: "{graph_key}" is wrong.' )

    # If no config is specified, use default config. 
    if config is None:
        config = cp.deepcopy(GRAPH_ATTRIBUTER_DEFAULT_PARAMS[feature_type])

    # Instantiate attributer class and call it, thereby attributing `so.G[spl][graph_key]`.
    attributer = GRAPH_ATTRIBUTER[feature_type]
    attributer(so, spl, graph_key, config)
